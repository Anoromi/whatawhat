...
use clap::Parser;
use whatawhat::{
    cli::subprocess_window_collector::run_collector, daemon::{args::DaemonArgs, start_daemon_with_intervals}, tauri::run_ui, utils::{
...
fn run(args: DaemonArgs) -> Result<()> {
    ...
    let collection_interval = args
        ...
        .map(|d| std::time::Duration::from_secs(d.num_seconds() as u64))
        .unwrap_or(std::time::Duration::from_secs(1));
    let idle_interval = args
        .idle_time
        .map(|d| std::time::Duration::from_secs(d.num_seconds() as u64))
        .unwrap_or(std::time::Duration::from_secs(5 * 60));
    run_ui(Some(app_dir), collection_interval, idle_interval)?;
