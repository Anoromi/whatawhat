...
use whatawhat_lib::{
    config::WatcherConfigBuilder, simple_cache::CacheConfig, ActiveWindowData,
    GenericWindowManager, WindowManager,
};
...
async fn run_collection_subprocess(
    ...
    command.args(&[
        "--collect-interval",
        &collection_interval.as_secs().to_string(),
    ]);
...
pub fn run_collector(
    ...
    let mut collector = GenericWindowManager::new(
        WatcherConfigBuilder::default()
            ...
            .idle_check_interval(idle_interval)
            .build()
...
impl WindowManager for SubprocessWindowManager {
    fn get_active_window_data(&mut self) -> Result<ActiveWindowData> {
        ...
        let current_data =
            futures::executor::block_on(async { self.current_data.lock().await.clone() });
